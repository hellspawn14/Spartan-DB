...
CREATE TABLE IF NOT EXISTS `Spartan`.`ParticipantGroups` (
  ...
  INDEX `FK_ParticipantGroup_Teams_idx` (`idParticipantTeam` ASC),
...
CREATE TABLE IF NOT EXISTS `Spartan`.`GroupTournamentEvents` (
  ...
  `nSpots` INT NOT NULL,
  `Key` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`idGroupEvent`, `idGroupTournament`),
  INDEX `idTournament_idx` (`idGroupTournament` ASC),
...
CREATE TABLE IF NOT EXISTS `Spartan`.`GroupTournamentParticipants` (
  ...
  CONSTRAINT `FK_GTParticipants_ParticipantGroups`
    FOREIGN KEY (`idParticipantTeam`)
    REFERENCES `Spartan`.`ParticipantGroups` (`idParticipantTeam`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
-- -----------------------------------------------------
-- Table `Spartan`.`PublicEvents`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `Spartan`.`PublicEvents` ;

CREATE TABLE IF NOT EXISTS `Spartan`.`PublicEvents` (
  `idPublicEvent` INT NOT NULL AUTO_INCREMENT,
...
-- -----------------------------------------------------
-- Table `Spartan`.`EventAssistence`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `Spartan`.`EventAssistence` ;

CREATE TABLE IF NOT EXISTS `Spartan`.`EventAssistence` (
  `idPublicEvent` INT NOT NULL,
  `idUserEvent` INT NOT NULL,
  `State` VARCHAR(45) NOT NULL,
  PRIMARY KEY (`idPublicEvent`, `idUserEvent`),
  INDEX `inUser_idx` (`idUserEvent` ASC),
  CONSTRAINT `inUser`
    FOREIGN KEY (`idUserEvent`)
    REFERENCES `Spartan`.`Users` (`idUser`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `inPublic`
    FOREIGN KEY (`idPublicEvent`)
    REFERENCES `Spartan`.`PublicEvents` (`idPublicEvent`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
START TRANSACTION;
USE `Spartan`;
INSERT INTO `Spartan`.`Users` (`idUser`, `UserName`, `PhoneNumber`) VALUES (1, 'hellspawn', '3172362158');
INSERT INTO `Spartan`.`Users` (`idUser`, `UserName`, `PhoneNumber`) VALUES (2, 'whogirl', '3045603797');
INSERT INTO `Spartan`.`Users` (`idUser`, `UserName`, `PhoneNumber`) VALUES (3, 'ClaudiaLJimnezG', '0313394949');
INSERT INTO `Spartan`.`Users` (`idUser`, `UserName`, `PhoneNumber`) VALUES (4, 'kirito', '3001234567');
INSERT INTO `Spartan`.`Users` (`idUser`, `UserName`, `PhoneNumber`) VALUES (5, 'Shinon', '3101234567');
...
START TRANSACTION;
USE `Spartan`;
INSERT INTO `Spartan`.`Events` (`idEvent`, `Sport`, `Description`, `Date`, `idEventAuthor`, `Spots`, `State`, `Place`, `Key`, `Details`) VALUES (1, 'Videogame', 'Let\'s play DOTA', '2014/10/31 22:00', 2, 1, 'Open', '51.508742,-0.120850', '1432', 'We\'re playing dota to have fun :) ');
...
START TRANSACTION;
USE `Spartan`;
INSERT INTO `Spartan`.`GroupTournaments` (`idTournament`, `Name`, `Description`, `Sport`, `State`, `idOrganizerGroup`, `nParticipants`) VALUES (1, 'Inaguration of Spartan', 'Tournament to conmemorate the creation of Spartan 5 App', 'Soccer', 'Open', 1, 4);
...
START TRANSACTION;
USE `Spartan`;
INSERT INTO `Spartan`.`SoloTournament` (`idTournament`, `Name`, `Description`, `Sport`, `State`, `idOrganizerSolo`, `nParticipants`) VALUES (1, 'Pokemon VGC', 'Let\'s Play Pokemon VGC Rules', 'Videogames', 'Open', 1, 4);
...
START TRANSACTION;
USE `Spartan`;
INSERT INTO `Spartan`.`SoloTournamentEvents` (`idSoloEvent`, `idSoloTournament`, `Date`, `State`, `Place`, `Details`, `nSpots`, `Key`) VALUES (1, 1, '2014/11/15', 'ToStart', '51.508742,-0.120850', 'The event will be held on the second floor', 2, '1234');
...
START TRANSACTION;
USE `Spartan`;
INSERT INTO `Spartan`.`GroupTournamentEvents` (`idGroupEvent`, `idGroupTournament`, `Date`, `State`, `Place`, `Details`, `nSpots`, `Key`) VALUES (1, 1, '2014/10/30 14:00', 'ToStart', '51.508742,-0.120850', 'The event will take place in the third floor', 2, '1234');
...
-- -----------------------------------------------------
-- Data for table `Spartan`.`PublicEvents`
-- -----------------------------------------------------
START TRANSACTION;
USE `Spartan`;
INSERT INTO `Spartan`.`PublicEvents` (`idPublicEvent`, `EventName`, `Sport`, `EventOrganizer`, `Place`, `Details`, `Date`) VALUES (1, 'Torneo SOFA', 'Videogame', 'Kurenai', 'Corferias xxxx', 'Traer 3DS', 'afawdgsaws');

COMMIT;


-- -----------------------------------------------------
-- Data for table `Spartan`.`EventAssistence`
-- -----------------------------------------------------
START TRANSACTION;
USE `Spartan`;
INSERT INTO `Spartan`.`EventAssistence` (`idPublicEvent`, `idUserEvent`, `State`) VALUES (1, 1, 'ToAssist');
